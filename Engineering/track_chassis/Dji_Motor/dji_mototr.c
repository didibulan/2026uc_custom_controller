...
#endif
// #ifdef USER_CAN_FD
    static bool Motor_Dji_Grouping(const uint32_t tx_id, const FDCAN_HandleTypeDef *can_handle, uint8_t tx_buff[8]) {
// #endif
    for(int i = 0; i < idx; i++) {
...
bool Motor_Dji_Control(DjiMotorInstance_s *motor, float taget) {
    ...
    if(motor->control_mode == DJI_POSITION){ // 位置控制模式
        ...
        motor->target_velocity = Pid_Calculate(motor->angle_pid, motor->velocity_pid, motor->target_position, motor->message.out_position, motor->message.out_velocity);
        motor->output = Pid_Calculate(motor->angle_pid, motor->velocity_pid, motor->target_velocity, motor->message.out_position, motor->message.out_velocity);
...
bool Motor_Dji_Transmit(const DjiMotorInstance_s *motor) {
    if(motor == NULL || motor->can_instance == NULL) {
        return false;
    }
